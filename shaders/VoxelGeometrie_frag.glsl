...
void	main() {
	...
	if (texID == 8) {
		float	largeNoise = noise3D(vec3(fragPos.x + time * 6.0f, fragPos.y + time * 12.0f, fragPos.z + time * 6.0f) * 0.05);
		largeNoise +=  noise3D(vec3(fragPos.x + time * 3.0f, fragPos.y + time * 12.0f, fragPos.z + time * 3.0f) * 0.05) * 0.5;
		largeNoise += sin(fragPos.x + time * 0.25) * 0.0125f;
